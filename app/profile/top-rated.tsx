...
type FilterKey = 'all' | 'album' | 'single';
export const options = { title: 'Top Rated' };
...
export default function TopRatedScreen() {
  ...
  const CACHE_KEY = 'top_rated_cache_v1';
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .limit(50);
    if (error) {
      setError('Could not load top rated');
      // eslint-disable-next-line no-console
      console.log('[top-rated] load error', error);
    } else if (data) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'album') return rows.filter(r => r.item_type === 'album');
    // treat singles as track
      ...
      {loading && rows.length === 0 ? (
        ...
        <View style={{ flex: 1 }}>
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={{ textAlign: 'center', marginTop: 20, color: colors.text.muted }}>{error ? 'Could not load. Pull to retry.' : 'No rated items yet.'}</Text>}
